...
static uint64_t pml4[1] __attribute__((aligned(0x1000)));
static uint64_t pdpt[1] __attribute__((aligned(0x1000))); 
static uint64_t page_directory[1] __attribute__((aligned(0x1000)));
...
void init_paging()
{
	...
	for (int i=0; i<0x200000; i+=0x1000) {
		kvmmap(&pml4[0], (void *)i, (void *)i, 0b11);
	}
...
{
	uint16_t pml4_index = ((uint64_t)virt >> 39) & 0b111111111;
	uint16_t pdpt_index = ((uint64_t)virt >> 30) & 0b111111111;
	uint16_t page_directory_index = ((uint64_t)virt >> 21) & 0b111111111;
	uint16_t page_table_index = ((uint64_t)virt >> 12) & 0b111111111;
	...
	pt[page_table_index] = ((uint64_t)phys) | (flags & 0xfff) | 0x1;
